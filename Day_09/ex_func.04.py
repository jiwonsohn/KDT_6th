...
# 함수이름:   check_data
# 매개변수:   str 데이터(예: '10 3'), 입력 데이터 수
# 함수결과:   True False
# ----------------------------------------------------

def check_data(user, valid_count):

    # 개수 체크
    if len(user.split()) == valid_count:

        decimalTrue=0
        for i in range(valid_count):
            if user.split()[i].isdecimal(): decimalTrue+=1
            else: continue
        
        if decimalTrue == valid_count: return True
        else: return False
    ...
    else: return "유효한 입력 개수가 아닙니다."


# TEST
# check_data( input("입력: "), 2)
...
def calc(func, op):
    ...
    if check_data(data, 2):
        ...
        data = data.split()
        print(f'\n결과: {data[0]} {op} {data[1]} = {func(int(data[0]),int(data[1]))} \n')
    else: print(f'{data}는 올바른 입력값이 아닙니다.')
